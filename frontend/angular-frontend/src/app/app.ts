...
export class App implements OnInit {
  ...
  getWsCellClass(r: number, c: number): string {
    ...
    let foundClass = '';
    // Simplify found checker for the manual angular port. 
    // In full implementation, we can track actual locations of found words.
  ...
  async sendChat() {
    ...
    if (this.geminiApiKey) {
      try {
        // Gemini API requires the first message in the conversation history to be from the 'user'.
        // So we filter out the initial hardcoded bot greeting.
        ...
        const payload = {
          systemInstruction: {
            parts: [{text: `You are an AI Legal Assistant for an educational platform about the Indian Constitution.

Goal:
Given a real-life situation (typed or voice input), identify relevant Fundamental Rights and Articles of the Indian Constitution and provide guidance in simple language.

INPUT:
User describes a situation (example: harassment, misuse of power, discrimination, etc.)

OUTPUT FORMAT:

1. Situation Summary
- Briefly understand and rewrite the user's problem in simple terms

2. Relevant Constitutional Articles
- List relevant Articles (e.g., Article 14, 19, 21)
- Give 1-line explanation for each
...
- Mention any legal restrictions (if applicable)

5. Safety Advice (IMPORTANT)
- If situation involves danger, harassment, or threat
- Suggest contacting police or helpline

6. Confidence Note
- Mention that this is educational guidance, not legal advice
...
- If the user asks in Hindi, you must reply in Hindi.`}]
        ...
        console.error(err);
        finalReply = 'Sorry, there was an error connecting to the AI model. Please verify your Gemini API key.';
      }
    } else {
      // Rule-based fallback
  ...
  getBotReplyEn(q: string) {
    ...
    if (l.includes('duty') || l.includes('duties')) return 'There are 11 Fundamental Duties under Article 51A. They remind citizens of their responsibilities.';
    if (l.includes('ambedkar')) return 'Dr. B.R. Ambedkar was the Chairman of the Drafting Committee and is known as the Father of the Indian Constitution.';
    if (l.includes('emergency')) return 'Emergency provisions are contained in Part XVIII, including National Emergency (Art. 352), State Emergency (Art. 356), and Financial Emergency (Art. 360).';
    if (l.includes('privacy')) return 'In the landmark 2017 Puttaswamy judgment, the Supreme Court declared the Right to Privacy as a Fundamental Right under Article 21.';
    return 'That is a great constitutional question! The Constitution is vast and detailed. What else would you like to explore?';
  }
  scrollToBottom() {
    setTimeout(() => {
